...
class Scheduler:
    ...
    def _compute_coeffs(self):
        self.betas = torch.tensor([self.get_beta(t) for t in range(self.T)], dtype=torch.float32).to(self.device)
    ...
    def add_noise(self, image: torch.Tensor):
        ...
        for t in range(self.T):
            noise = torch.randn_like(noisy_images[-1]).to(self.device)
...
class CosineScheduler(Scheduler):
    def __init__(self, T, **kwargs):
        ...
        cos_values = torch.tensor([
            self._cosine_decay_function(torch.tensor(t, dtype=torch.float32).to(self.device))
    ...
    def _cosine_decay_function(self, t):
        ...
        return (torch.cos(((t / self.T + self.s) / (1 + self.s)) * torch.pi / 2) ** 2).to(self.device)
...
class LinearScheduler(Scheduler):
    def __init__(self, T, start=0.0001, end=0.02, **kwargs):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.betas = torch.linspace(start, end, T, dtype=torch.float32).to(self.device)
        super().__init__(T, **kwargs)
